...
**Local dev:** `npm run dev` uses `VITE_API_URL=http://localhost:3000/api` from `.env` unless overridden. Run `npm run sync-kiosk` after changing files under `../pi/kiosk` so `public/kiosk/` stays in sync.
